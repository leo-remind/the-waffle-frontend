  ...
  const connectWebSocket = (queryText: string) => {
    ...
    socket.onmessage = (event) => {
      ...
      if (message["isStreaming"]) {
        ...
        // Highlight: Add SQL reasoning to chat history if verboseParam is true and sql_reasoning exists
        if (verboseParam && message["sqlReasoning"]) {
          newChatHistory.push({
            role: "system",
            value: message["sqlReasoning"]
          } as ChatHistory);
        }
        ...
        {results.length > 0 && (
          <div className="message-container">
            ...
            <div className="text-container">
              <div
                key={results[0]}
                className="animate-float text-base text-primary font-bold font-dm-sans text-md"
              >
                {results[0]}
              </div>
            </div>
          ...
          <div className="flex justify-between items-center px-5 py-3">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              <SpeechToText
                ...
                languageCode={language == "hindi" ? "hi" : "en"}
              />
              <Button
                size="icon"
                className={`rounded-full ${
                  isProcessing ? "bg-secondary animate-pulse" : "bg-primary"
